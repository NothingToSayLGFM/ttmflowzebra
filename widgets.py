...
class TTNRow(ctk.CTkFrame):
    ...
    def __init__(self, parent, index: int, ttn: str, on_retry=None):
        super().__init__(parent, fg_color=C_DARK, corner_radius=6)
        self.grid_columnconfigure(3, weight=1)
        self._on_retry = on_retry

        ctk.CTkLabel(
            self, text=f"#{index}", width=36,
        ...
        self.dot = ctk.CTkLabel(self, text="●", text_color=C_GRAY, width=16)
        self.dot.grid(row=0, column=4, padx=(0, 4))

        self.lbl = ctk.CTkLabel(
            self, text="Очікує", width=150, anchor="e",
            font=ctk.CTkFont(size=12), text_color=C_GRAY
        )
        ...
        self._retry_btn.grid(row=0, column=6, padx=(0, 10))
    ...
    def add_sub_ttns(self, sub_ttns: list[str]):
        ...
        sub_frame.grid(row=1, column=0, columnspan=6, sticky="ew", padx=(52, 10), pady=(0, 8))
        for i, ttn in enumerate(sub_ttns):
            ...
            ctk.CTkButton(
                sub_frame, text="⎘", width=28, height=22,
                fg_color="#444444", hover_color="#555555",
                font=ctk.CTkFont(size=12),
                command=lambda t=ttn: (self.clipboard_clear(), self.clipboard_append(t))
            ).grid(row=i, column=1, padx=(8, 0))
...
class RegistryCard(ctk.CTkFrame):
    def __init__(self, parent, group: dict):
        ...
        name_row.grid(row=0, column=0, padx=14, pady=(12, 2), sticky="ew")
        ...
        self._name = name
        ctk.CTkButton(
            name_row, text="⎘", width=32, height=28,
            fg_color="#444444", hover_color="#555555",
        ...
        ctk.CTkLabel(
            ...
            font=ctk.CTkFont(size=12), text_color=C_GRAY, anchor="w", wraplength=420
        ...
        self.count_lbl.grid(row=2, column=0, padx=14, pady=(0, 6), sticky="w")

        self.status_lbl = ctk.CTkLabel(
            self, text="⏳  Очікування перевірки",
            font=ctk.CTkFont(size=13), text_color=C_GRAY, anchor="w"
        )
        self.status_lbl.grid(row=3, column=0, padx=14, pady=(0, 12), sticky="w")
        self._next_ttn_row = 4
    ...
    def set_done(self, done: int, err: int, ttns: list[str] = None):
        ...
        if ttns:
            for i, ttn in enumerate(ttns):
                ...
                row_frame.grid(row=self._next_ttn_row + i, column=0, padx=28, pady=(0, 2), sticky="w")
                ctk.CTkLabel(
                    row_frame, text=ttn,
                    font=ctk.CTkFont(family="Consolas", size=12), text_color=C_GREEN, anchor="w"
                ).pack(side="left")
                ctk.CTkButton(
...
class PrintedModal(ctk.CTkToplevel):
    ...
    def _build_ui(self):
        ...
        ctk.CTkEntry(
            ...
            height=36, font=ctk.CTkFont(family="Consolas", size=14)
        ...
        self._row_idx = 0
        self._row_widgets: list[tuple[str, ctk.CTkFrame]] = []
    ...
    def _add_ttns(self, ttns: list):
        ...
        for ttn in ttns:
            ...
            ctk.CTkLabel(
                row_frame, text=ttn,
                font=ctk.CTkFont(family="Consolas", size=15, weight="bold"), anchor="w"

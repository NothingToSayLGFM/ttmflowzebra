...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.ttn_rows: dict[int, TTNRow] = {}
        self.ttn_indices: dict[str, list[int]] = {}
        self.groups:     dict = {}
        self.all_groups: dict = {}

        self.all_chunks: list[list[str]] = []
        self.all_ttns:   list[str] = []
        self.selected_chunk_var = tk.IntVar(value=0)
        self.done_reg_rows: int = 0
        self._next_ttn_grid_row: int = 0
        self._stop_analysis = threading.Event()
        self.all_reg_cards: dict = {}
        ...
        self._scan_tempfile: str = ""   # path to temp TTN file
        self._active_tab: str = 'ttn'
    ...
    def _cleanup_tempfile(self):
        if self._scan_tempfile and os.path.exists(self._scan_tempfile):
            try:
                os.unlink(self._scan_tempfile)
            except Exception:
                pass
        self._scan_tempfile = ""
    ...
    def _build_ui(self):
        ...
        # ── Екран вибору режиму ──
        self._mode_frame = ctk.CTkFrame(self, fg_color="transparent")
        self._mode_frame.grid(row=0, column=0, sticky="nsew")
        self._build_mode_screen()

        # ── Основний екран (схований спочатку) ──
    ...
    def _build_mode_screen(self):
        ...
        ctk.CTkLabel(
            f, text="Виберіть режим роботи:",
            font=ctk.CTkFont(size=15), text_color=C_GRAY
        ).grid(row=1, column=0, pady=(0, 24))

        btn_frame = ctk.CTkFrame(f, fg_color="transparent")
        btn_frame.grid(row=2, column=0, sticky="ew", padx=48)
        btn_frame.grid_columnconfigure(0, weight=1)

        ctk.CTkButton(
            btn_frame, text="📷  Сканую зараз", height=64,
            font=ctk.CTkFont(size=18),
            fg_color=C_BLUE, hover_color="#2471a3",
            command=lambda: self._pick_mode('scanner')
        ).grid(row=0, column=0, sticky="ew", pady=(0, 16))

        ctk.CTkButton(
            btn_frame, text="📁  З файлу", height=64,
            font=ctk.CTkFont(size=18),
            fg_color="#444444", hover_color="#555555",
            command=lambda: self._pick_mode('file')
        ).grid(row=1, column=0, sticky="ew")

        ctk.CTkFrame(f, height=1).grid(row=3, column=0)  # spacer bottom
    ...
    def _build_main_screen(self):
        ...
        ctk.CTkButton(
            icon_frame, text="🖨", width=40, height=40,
            fg_color="#444444", hover_color="#555555",
            font=ctk.CTkFont(size=18),
        ...
        ctk.CTkButton(
            ...
            fg_color="#444444", hover_color="#555555",
            font=ctk.CTkFont(size=18),
            command=self._back_to_mode_select
        ).pack(side="left")

        # ── Вкладки ТТН / Реєстри ──
        tab_bar = ctk.CTkFrame(f, fg_color="transparent")
        tab_bar.grid(row=1, column=0, sticky="ew", padx=10, pady=(8, 0))
        tab_bar.grid_columnconfigure(0, weight=1)
        tab_bar.grid_columnconfigure(1, weight=1)

        self._tab_ttn_btn = ctk.CTkButton(
            tab_bar, text="ТТН", height=40,
            font=ctk.CTkFont(size=15, weight="bold"),
            fg_color=C_TAB_ACTIVE, hover_color="#2471a3",
            corner_radius=6,
            command=lambda: self._switch_tab('ttn')
        )
        self._tab_ttn_btn.grid(row=0, column=0, sticky="ew", padx=(0, 3))

        self._tab_reg_btn = ctk.CTkButton(
            tab_bar, text="Реєстри", height=40,
            font=ctk.CTkFont(size=15, weight="bold"),
            fg_color=C_TAB_INACTIVE, hover_color="#444444",
            corner_radius=6,
            command=lambda: self._switch_tab('registry')
        )
        self._tab_reg_btn.grid(row=0, column=1, sticky="ew", padx=(3, 0))

        # ── Поле сканера (тільки в режимі scanner) ──
        self._scan_entry_frame = ctk.CTkFrame(f, fg_color="transparent")
        self._scan_entry_frame.grid(row=2, column=0, sticky="ew", padx=10, pady=(8, 0))
        self._scan_entry_frame.grid_columnconfigure(0, weight=1)

        self._scan_entry = ctk.CTkEntry(
            self._scan_entry_frame,
            placeholder_text="Скануй тут...",
            height=56,
            font=ctk.CTkFont(family="Consolas", size=18),
        )
        self._scan_entry.grid(row=0, column=0, sticky="ew")
        self._scan_entry.bind("<Return>", self._on_scan_entry)
        self._scan_entry_frame.grid_remove()   # захований до вибору режиму

        # ── Контентна область (TTN list + Reg list, одне одночасно) ──
        content = ctk.CTkFrame(f, fg_color="transparent")
        content.grid(row=3, column=0, sticky="nsew", padx=10, pady=(8, 0))
        content.grid_columnconfigure(0, weight=1)
        content.grid_rowconfigure(0, weight=1)

        self.ttn_list = ctk.CTkScrollableFrame(content, label_text="ТТН")
        self.ttn_list.grid(row=0, column=0, sticky="nsew")
        self.ttn_list.grid_columnconfigure(0, weight=1)

        self.reg_list = ctk.CTkScrollableFrame(content, label_text="Реєстри для створення")
        self.reg_list.grid(row=0, column=0, sticky="nsew")
        self.reg_list.grid_columnconfigure(0, weight=1)
        self.reg_list.grid_remove()   # захований — активна вкладка ТТН

        # ── Нижні кнопки ──
        bottom = ctk.CTkFrame(f, fg_color="transparent")
        bottom.grid(row=4, column=0, sticky="ew", padx=10, pady=(6, 6))
        bottom.grid_columnconfigure(0, weight=1)

        self.analyze_btn = ctk.CTkButton(
            bottom, text="Аналізувати", height=48,
            font=ctk.CTkFont(size=16),
            fg_color=C_BLUE, hover_color="#2471a3",
            command=self._analyze
        )
        self.analyze_btn.grid(row=0, column=0, sticky="ew", pady=(0, 5))

        self.analyze_all_btn = ctk.CTkButton(
            bottom, text="Аналізувати все", height=36,
            font=ctk.CTkFont(size=13),
            fg_color="#1a7a3c", hover_color="#145e2e",
            command=self._analyze_all
        )
        self.analyze_all_btn.grid(row=1, column=0, sticky="ew", pady=(0, 5))

        self.distribute_btn = ctk.CTkButton(
            bottom, text="Авторозподіл", height=48,
            font=ctk.CTkFont(size=16),
            fg_color=C_GREEN, hover_color="#1e8449",
            state="disabled", command=self._distribute
        )
        self.distribute_btn.grid(row=2, column=0, sticky="ew", pady=(0, 4))
    ...
    def _pick_mode(self, mode: str):
        ...
        if mode == 'scanner':
            ...
            tf.close()
            self._scan_tempfile = tf.name
            self.input_file.set(tf.name)
            self._scan_entry_frame.grid()
            self.after(100, lambda: self._scan_entry.focus_set())
        else:
            self._scan_entry_frame.grid_remove()
    ...
    def _switch_tab(self, tab: str):
        ...
        if tab == 'ttn':
            self.ttn_list.grid()
            self.reg_list.grid_remove()
            self._tab_ttn_btn.configure(fg_color=C_TAB_ACTIVE)
            self._tab_reg_btn.configure(fg_color=C_TAB_INACTIVE)
        else:
            self.ttn_list.grid_remove()
            self.reg_list.grid()
            self._tab_ttn_btn.configure(fg_color=C_TAB_INACTIVE)
            self._tab_reg_btn.configure(fg_color=C_TAB_ACTIVE)

    # ── Поле сканера ──────────────────────────────────────

    def _on_scan_entry(self, event=None):
        ttn = self._scan_entry.get().strip()
        if ttn and self._scan_tempfile:
            with open(self._scan_tempfile, 'a', encoding='utf-8') as fp:
                fp.write(ttn + '\n')
        self._scan_entry.delete(0, 'end')
        return "break"
    ...
    def _open_settings(self):
        ...
        ctk.CTkEntry(frm, textvariable=api_var, height=36, show="*").grid(
            row=0, column=1, columnspan=2, sticky="ew", padx=(8, 0), pady=(0, 10)
        )

        # Шлях до файлу відображаємо тільки в режимі 'file'
        if self._mode == 'file':
            ctk.CTkLabel(frm, text="Файл ТТН:", width=80, anchor="w").grid(row=1, column=0)
            ctk.CTkEntry(frm, textvariable=file_var, height=36).grid(
            ...
            ctk.CTkButton(frm, text="...", width=40, height=36, command=_browse).grid(row=1, column=2)
        ...
        ctk.CTkButton(btn_frm, text="Зберегти", width=110, height=38,
                      fg_color=C_BLUE, hover_color="#2471a3",
                      command=_save).pack(side="left", padx=(0, 8))
        ctk.CTkButton(btn_frm, text="Відміна", width=110, height=38,
    ...
    def _render_ttn_chunk(self, chunk_n: int, chunk_ttns: list, file_ttn_set: set,
        ...
        ctk.CTkRadioButton(
            ...
            text_color="#aaaaaa",
        ).grid(row=grid_row, column=0, sticky="w", padx=10, pady=(2, 2))
        grid_row += 1
    ...
    def _start_analyze_after_balance_check(self, api_key: str, input_file: str):
        ...
        # Переключитись на вкладку ТТН щоб бачити прогрес
        self._switch_tab('ttn')

        def _worker(ttns=chunk, api_key=api_key, start=chunk_start):
          try:
            ok_pairs: dict[str, tuple[str, dict]] = {}
            ok_indices: dict[str, list[int]] = {}
    ...
    def _handle_analysis_done(self, groups: dict, canonical: dict, parent_sub_map: dict, scanned_count: int = 0):
        ...
        self._status(f"Порція {sel + 1}/{n_chunks} проаналізована. Реєстри: {', '.join(names)}.")
        next_chunk = sel + 1
        if next_chunk < n_chunks:
            self.selected_chunk_var.set(next_chunk)
        if self._analyze_all_mode:
    ...
    def _handle_distribute_done(self, results: dict):
        ...
        # Автоматично переключитись на вкладку Реєстри
        self._switch_tab('registry')

        if next_chunk < n_chunks:
            self.selected_chunk_var.set(next_chunk)
            self.analyze_btn.configure(state="normal", text="Аналізувати")
            self._status("Розподіл завершено.")
        else:
            self.selected_chunk_var.set(0)
            self.analyze_btn.configure(state="normal", text="Аналізувати")
    ...
    def _show_np_connection_error_popup(self):
        ...
        ctk.CTkButton(
            popup, text="OK", width=100, height=36,
    ...
    def _show_warning_popup(self, message: str, reply_event: threading.Event):
        ...
        ctk.CTkButton(
            btn_frame, text="Зупинити", width=110, height=36,
            fg_color="#555555", hover_color="#444444",
            command=_stop
        ).pack(side="left")
    ...
    def _save_config(self):
        ...
        # Зберігаємо шлях до файлу тільки в режимі 'file' (не temp файл сканера)
        if self._mode == 'file':
